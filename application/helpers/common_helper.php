...
function user_login() { redirect("account/login"); }
function user_home() { redirect("user/home"); }
function user_logout() { redirect("account/logout"); }

function supplier_login() { redirect("account_supplier/login"); }
function supplier_home() { redirect("supplier/home"); }
function supplier_logout() { redirect("account_supplier/logout"); }
...
function get_user($umail,$upass)
{
	...
	if(emptyres($res)) {
		$q = "SELECT * FROM users WHERE twitter_id = ? AND password = ?";
		$res = $CI->db->query($q,array($umail,$upass)); 
		if(emptyres($res)) return false;
		else return $res->row();
	}
...
function set_login_session($umail,$upass,$type="admin")
{
	...
	if($type=="supplier") $CI->session->set_userdata(array("sname"=>$umail, "spass"=>$upass));
...
function unset_login_session($type="admin")
{
	...
	if($type=="supplier") $CI->session->unset_userdata(array("sname"=>"", "spass"=>""));
...
//TAMBAH SESSION SUPPLIER
function get_logged_in_supplier()
{ 
	$CI =& get_instance();
	if($CI->session->userdata('sname') == false || $CI->session->userdata('spass') == false) return false;
	return get_supplier($CI->session->userdata('sname'),$CI->session->userdata('spass')); 
}
function get_supplier($smail,$spass)
{
	$CI =& get_instance();
	$q = "SELECT * FROM suppliers WHERE email = ? AND password = ?";
	$res = $CI->db->query($q,array($smail,$spass)); 
	if(emptyres($res)) return false;
	else return $res->row();
}
