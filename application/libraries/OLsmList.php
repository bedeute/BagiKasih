...
class OLsmList
{
	...
	public static function get_list($page=0, $limit=0, $orderby="id_lsm DESC")
	{
		$CI =& get_instance();
	...
	public function get_supplier_product_sizes($page=0, $limit=0/*, $orderby="id DESC", $active=""*/)
	{
		...
		*/
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS p.*, sps.* FROM supplier_product_sizes sps, lsm_list p WHERE sps.product_id = p.id {$add_sql_stats} ", $sql_arrs);
		if(emptyres($res)) return array();
		else return $res->result();
	}
	...
	{
		$photo = OLsmListPhoto::get_list(0, intval($limit), "id DESC", $this->id);
					
		$img_url = NULL;
		foreach($photo as $row)
		{
			$PP = new OLsmListPhoto();
			$PP->setup($row);
			
			$img_url = $PP->get_photo($row->image, "lsm_list", $size);
	...
	public function get_product_by_category($page=0, $limit=0, $orderby="p.id DESC", $category_id="")
	{
		...
		*/
		...
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS p.*, p.name as product_name, c.*, b.*, b.name as brand_name, ps.*, s.*, s.name as size_name
							  	FROM lsm_list p, categories c, brands b, product_sizes ps, sizes s 
	...
	public function get_link()
	{
		...
		if($this->id == "") return "";
		return "lsm_list/details/".$this->id;		
	}
	...
	public function get_supplier_product_size_details($location_id="", $page=0, $limit=0, $orderby="p.id DESC")
	{
		...
		else
		{
			...
			foreach($res->result() as $row)
			{
				$osps = new OSupplierlsm_listize($row->id);
