...
class OTypecategory
{
	...
	public function __construct($id, $type="id")
	{
		...
		else
		{
			if($type == "id")
			{
				$q = "SELECT * FROM type_categories WHERE id = ?";
			}
			else 
			{
				$q = "SELECT * FROM type_categories WHERE url_title = ?";
			}
	...
	{
		$CI =& get_instance();
		$CI->db->insert('type_categories',$params);
		...
		if($new_id)
		{
			if(trim($params['name']) != "")
			{
				...
				$check_duplicate = $CI->db->query("SELECT id FROM type_categories WHERE url_title = ?", array($url_title));
	...
	public function edit($params)
	{
		if(trim($params['name']) != "")
		{
			...
			$check_duplicate = $this->db->query("SELECT id FROM type_categories WHERE url_title = ? AND id <> ?", array($url_title, $this->id));
			if(!emptyres($check_duplicate))
			{
		...
		return $this->db->update('type_categories',$params,array('id' => $this->id));		
	}
	
	public function delete()
	{
		return $this->db->delete('type_categories',array('id' => $this->id));		
	}	
	...
	public static function get_list($page=0, $limit=0, $orderby="id DESC",$category_id = 0)
	{
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM type_categories {$add_sql_stats} ", $sql_arrs);
	...
	{
		$lists = OTypecategory::get_list(0, 0, "name ASC", "0");
		$arr = array();
	...
	{
		$lists = OTypecategory::get_list(0, 0, "name ASC", $category_id);
	...
	public static function get_type_ddl($name, $selval, $optional, $category_id, $default_value = '')
	{
		$list = OTypecategory::get_list(0, 0, "id ASC", $category_id);
