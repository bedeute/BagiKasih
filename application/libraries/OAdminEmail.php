...
class OAdminEmail
{
	...
	public function __construct($id, $type="id")
	{
		...
		else
		{
			if($type == "id")
			{
				$q = "SELECT * FROM admin_emails WHERE id = ?";
			}
			else 
			{
				$q = "SELECT * FROM admin_emails WHERE url_title = ?";
			}
	...
	{
		$CI =& get_instance();
		$CI->db->insert('admin_emails',$params);
		return $CI->db->insert_id();
	}
	
	public function edit($params)
	{
		return $this->db->update('admin_emails',$params,array('id' => $this->id));		
	}
	
	public function delete()
	{
		return $this->db->query("DELETE FROM admin_emails WHERE id = ?", array($this->id));		
	}
	/*
	 * EXTERNAL FUNCTIONS
	 */
	...
	public static function get_list($page=0, $limit=0, $orderby="id DESC"/*, $admin_id=""*/)
	{
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM admin_emails {$add_sql_stats} ", $sql_arrs);
	...
	{
		$list = OAdminEmail::get_list(0, 0, "email ASC");
	...
	{
		$ci =& get_instance();
		$q = "SELECT * FROM admin_emails WHERE email = ? AND id != ? LIMIT 1";
