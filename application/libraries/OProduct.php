...
class OProduct
{
	...
	public function __construct($id)
	{
		...
		{
			$q = "SELECT * FROM products WHERE id_product = ?";
	...
	{
		$CI =& get_instance();
		$CI->db->insert('products',$arr);
		return $CI->db->insert_id();
	}
	
	public function edit($arr)
	{
		return $this->db->update('products',$arr,array('id_product' => $this->id));		
	}
	
	public function delete()
	{
		return $this->db->query("DELETE FROM products WHERE id_product = ?", array($this->id));
	}	
	
	public static function get_list($page=0, $limit=0, $orderby="id_product DESC")
	{
		$CI =& get_instance();
	...
	{
		$list = OProduct::get_list(0, 0, "name ASC");
		
		foreach($list as $r)
		{
			$arr[$r->id_product] = $r->name;			
		}
	...
	public function get_filtered_list_for_gift($brand_id = 0,$category_id = 0, $subcat_id = 0, $size_id = 0, $topic_id = 0,$orderby = "p.name ASC",$start = 0, $limit = 20, $featured = 0, $type_id = 0, $theme_id = 0, $price_range_id = 0)
	{
	...
	public function get_supplier_product_sizes($page=0, $limit=0/*, $orderby="id DESC", $active=""*/)
	{
		...
		*/
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS p.*, sps.* FROM supplier_product_sizes sps, products p WHERE sps.product_id = p.id {$add_sql_stats} ", $sql_arrs);
		if(emptyres($res)) return array();
		else return $res->result();
	}
	...
	{
		$photo = OProductPhoto::get_list(0, intval($limit), "id DESC", $this->id);
		...
		foreach($photo as $row)
		{
			...
			$img_url = $PP->get_photo($row->image, "products", $size);
	...
	public function get_product_by_category($page=0, $limit=0, $orderby="p.id DESC", $category_id="")
	{
		...
		*/
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS p.*, p.name as product_name, c.*, b.*, b.name as brand_name, ps.*, s.*, s.name as size_name
							  	FROM products p, categories c, brands b, product_sizes ps, sizes s 
	...
	public function get_link()
	{
		...
		if($this->id == "") return "";
		return "products/details/".$this->id;		
	}
	
	
	
	
	public function get_filtered_list_by_price($location_id,$brand_id = 0,$category_id = 0, $subcat_id = 0, $size_id = 0, $topic_id = 0,$orderby = "p.name ASC",$start = 0, $limit = 20, $featured = 0, $start_price = 0, $end_price = 0)
	{
		// get all suppliers
		$q = "SELECT * FROM suppliers WHERE location_id = ?";
		$res = $this->db->query($q,array($location_id));
	...
	public function get_supplier_product_size_details($location_id="", $page=0, $limit=0, $orderby="p.id DESC")
	{
		...
		else
		{
			...
			foreach($res->result() as $row)
			{
				$osps = new OSupplierProductSize($row->id);
